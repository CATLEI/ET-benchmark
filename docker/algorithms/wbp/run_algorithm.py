...
def main():
    ...
    try:
        ...
        # Hyperspy can return a list of signals; take the first
        ...
        # WBP expects Signal2D (tilt series: n_tilts x height x width). If loaded as Signal1D
        # (e.g. due to axis metadata), build a Signal2D from the data when shape is correct.
        if not isinstance(tilt_series, hs.signals.Signal2D):
            if hasattr(tilt_series, "data") and tilt_series.data.ndim >= 2:
                ...
                print(f"ERROR: Expected Signal2D or 2D/3D data, got {type(tilt_series)}", file=sys.stderr)
                sys.exit(1)
        
        # Initialize and run WBP algorithm
